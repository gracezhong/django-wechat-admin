	...
	export default {
			...
			logout: function () {
				...
				this.$confirm('确认退出吗?', '提示', {
				    ...
				    logOut(para).then((res)=> {
                        ...
                        console.log('Close eventSource listener');
		...
		mounted() {
			...
			if (user) {
                ...
                this.eventSource = this.$eventSourceListener(user.puid);
                console.log('mounted', this.eventSource);
				this.sysUserName = user.nick_name || '';
				this.sysUserAvatar = 'http://127.0.0.1:8000' + user.avatar || '';
